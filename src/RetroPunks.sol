...
contract RetroPunks is ERC721SeaDropPausableAndQueryable, FisherYatesShuffler {
    ...
    function setTokenBackground(uint tokenId, uint backgroundIndex) public onlyTokenOwner(tokenId) notSpecial(tokenId) {
        require(
    ...
    function setTokenName(uint tokenId, string memory name) public onlyTokenOwner(tokenId) notSpecial(tokenId) {
    ...
    function resetTokenName(uint256 tokenId) public onlyTokenOwner(tokenId) notSpecial(tokenId) {
    ...
    function revealShufflerSeed(uint256 _seed, uint256 _nonce) external onlyOwner {
        ...
        require(keccak256(abi.encodePacked(_seed, _nonce)) == committedShufflerSeedHash, "Result does not match committedShufflerSeedHash");
    ...
    function renderDataUri(uint256 _tokenId, uint16 _tokenIdSeed, uint16 _backgroundIndex, string memory _name, uint256 _globalSeed) internal view returns (string memory) {
        ...
        string memory displayName = keccak256(bytes(_name)) == keccak256(bytes(string.concat("RetroPunk #", Utils.toString(_tokenId))))
            ? _name // RetroPunk #{_tokenId}
            : string.concat("RetroPunk #", Utils.toString(_tokenId), ": ", _name); // RetroPunk #{_tokenId}: {_name}

...
contract RetroPunks is ERC721SeaDropPausableAndQueryable {
    ...
    string[7] private SPECIAL_NAMES = ["Predator Blue", "Predator Green", "Predator Red", "Santa Claus", "Shadow Ninja", "The Devil", "The Portrait"];
    ...
    constructor(ISVGRenderer _rendererParam, bytes32 _committedGlobalSeedHashParam, bytes32 _committedShufflerSeedHashParam, uint256 _maxSupplyParam, address[] memory allowedSeaDropParam) ERC721SeaDropPausableAndQueryable("RetroPunks", "RPNKS", allowedSeaDropParam) {
    ...
    function setMaxSupply(uint256 newMaxSupply) external override(ERC721ContractMetadata, ISeaDropTokenContractMetadata) onlyOwner {
        if (_totalMinted() < _maxSupply) {
            revert Mint_Total_Supply_Before_Setting_Max_Supply();
        }
    ...
    function setTokenName(uint256 tokenId, string calldata name) external onlyTokenOwner(tokenId) notSpecial(tokenId) {
        ...
        for (uint256 i = 0; i < b.length;) {
            bytes1 c = b[i];
            if (!((c >= 0x30 && c <= 0x39) || (c >= 0x41 && c <= 0x5A) || (c >= 0x61 && c <= 0x7A) || (c == 0x20 || c == 0x21 || c == 0x2D || c == 0x2E || c == 0x3F || c == 0x5F || c == 0x26 || c == 0x27))) revert InvalidCharacterInName();
            unchecked {
                ++i;
            }
        }
    ...
    function setTokenBio(uint256 tokenId, string calldata bio) external onlyTokenOwner(tokenId) notSpecial(tokenId) {
    ...
    function _saveNewSeed(uint256 tokenId, uint256 remaining) internal {
        ...
        globalTokenMetadata[tokenId] = TokenMetadata({tokenIdSeed: uint16(newTokenIdSeed), backgroundIndex: defaultBackgroundIndex, name: _getInitialName(tokenId, uint16(newTokenIdSeed)), bio: "A RetroPunk living on-chain."});
    }
    ...
    function renderDataUri(uint256 _tokenId, uint16 _tokenIdSeed, uint8 _backgroundIndex, string memory _name, string memory _bio, uint256 _globalSeed) internal view returns (string memory) {
        string memory svg;
        string memory attributes;
        (svg, attributes) = renderer.renderSVG(_tokenIdSeed, _backgroundIndex, _globalSeed);

        string memory displayName = keccak256(bytes(_name)) == keccak256(bytes(string.concat("#", Utils.toString(_tokenId)))) ? _name : string.concat("#", Utils.toString(_tokenId), ": ", _name);

        string memory json = string.concat('{"name":"', displayName, '","description":"', _bio, '",', attributes, ",", '"image":"data:image/svg+xml;base64,', Utils.encodeBase64(bytes(svg)), '"}');

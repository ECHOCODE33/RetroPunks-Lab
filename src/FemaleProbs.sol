// SPDX-License-Identifier: MIT
pragma solidity ^0.8.30;

import { IFemaleProbs } from './interfaces/IFemaleProbs.sol';
import { E_Female_Skin, E_Female_Eyes, E_Female_Face, E_Female_Chain, E_Female_Earring, E_Female_Scarf, E_Female_Mask, E_Female_Hair, E_Female_Hat_Hair, E_Female_Headwear, E_Female_Eye_Wear } from "./common/Enums.sol";
import { TraitsContext} from './common/Structs.sol';
import { RandomCtx, Random } from "./libraries/Random.sol";
import { TraitsLib } from "./libraries/TraitsLib.sol";

/**
 * @author ECHO
 */

contract FemaleProbs is IFemaleProbs {

    function selectFemaleSkin(RandomCtx memory rndCtx) external pure returns (E_Female_Skin) {
        bytes memory packed = hex"0000001900E1000F002D0041001402EE02EE02EE02EE02EE02EE02EE02EE02EE02EE02EE02EE00080019005A0016000A012C0096";
        return E_Female_Skin(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectFemaleEyes(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Female_Eyes) {
        if (TraitsLib.femaleIsGhost(ctx)) {
            return Random.randBool(rndCtx, 5000) ? E_Female_Eyes.Ghost_Left : E_Female_Eyes.Ghost_Right;
        }

        bytes memory packed = hex"000000320064000B000B000B000B000B000B000B000B000B00FA001E001E001E001E001E001E001E001E101D00190019101D007D01F401F4004B";
        return E_Female_Eyes(Random.selectRandomTrait(rndCtx, packed, 10239));
    }

    function selectFemaleFace(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Female_Face) {
        bytes memory packed;
        uint256 totalWeight;
        
        if (TraitsLib.femaleIsHuman(ctx) || TraitsLib.femaleIsZombie(ctx)) {
            // Human/Zombie weights
            packed = hex"24A10010000C004B0010007D0023000A000A00080012000A0016000C00C8000C0014000E0008";
            totalWeight = 10000;
        } 

        else if (TraitsLib.femaleIsApe(ctx) || TraitsLib.femaleIsZombieApe(ctx)) {
            // Ape weights (Only Mole, Bandage, War Paint, Bionics, Cybereyes)
            packed = hex"25C100000000004B001000000000000A000A0000000000000000000C00C8000C000000000000";
            totalWeight = 10000;
        } 

        else if (TraitsLib.femaleIsAlien(ctx) || TraitsLib.femaleIsDemon(ctx) || TraitsLib.femaleIsGhost(ctx) || TraitsLib.femaleIsGlitch(ctx) || TraitsLib.femaleIsGoblin(ctx) || TraitsLib.femaleIsSkeleton(ctx) || TraitsLib.femaleIsVampire(ctx)) {
            // All other weights (Only Mole, Tattoos, Bandage)
            packed = hex"25620010000C004B0000000000230000000000000012000A0016000000C800000014000E0008";
            totalWeight = 10000;
        }

        else {
            return E_Female_Face.None;
        }

        return E_Female_Face(Random.selectRandomTrait(rndCtx, packed, totalWeight));
    }

    function selectFemaleChain(RandomCtx memory rndCtx) external pure returns (E_Female_Chain) {
        bytes memory packed = hex"232800FA0019004B008C015E000A00320064";
        return E_Female_Chain(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectFemaleEarring(RandomCtx memory rndCtx) external pure returns (E_Female_Earring) {
        bytes memory packed = hex"232800FA0019004B008C015E000A00320064";
        return E_Female_Earring(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectFemaleScarf(RandomCtx memory rndCtx) external pure returns (E_Female_Scarf) {
        bytes memory packed = hex"26AC0028001E001E";
        return E_Female_Scarf(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectFemaleMask(RandomCtx memory rndCtx) external pure returns (E_Female_Mask) {
        bytes memory packed = hex"253F001E001E001E001E001E002800190019001900190019001900190019000F000A000A000A000A000A";
        return E_Female_Mask(Random.selectRandomTrait(rndCtx, packed, 9990));
    }

    function selectFemaleHair(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Female_Hair) {
        uint256 noneWeight;
        
        if (TraitsLib.femaleIsRobot(ctx) || TraitsLib.femaleIsMummy(ctx) || TraitsLib.femaleIsVampire(ctx)) {
            return E_Female_Hair.None;
        }

        else if (TraitsLib.femaleIsHuman(ctx) || TraitsLib.femaleIsZombie(ctx)) {
            noneWeight = 400;
        }

        else {
            noneWeight = 1200;
        }

        uint256 total = 9595 + noneWeight;
        
        if (Random.randRange(rndCtx, 1, total) <= noneWeight) {
            return E_Female_Hair.None;
        }

        bytes memory packed = hex"";
        return E_Female_Hair(Random.selectRandomTrait(rndCtx, packed, 9595));
    }

    function selectFemaleHatHair(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Female_Hat_Hair) {
        uint256 noneWeight;
        
        if (TraitsLib.femaleIsRobot(ctx) || TraitsLib.femaleIsMummy(ctx) || TraitsLib.femaleIsVampire(ctx)) {
            return E_Female_Hat_Hair.None;
        }

        else if (TraitsLib.femaleIsHuman(ctx) || TraitsLib.femaleIsZombie(ctx)) {
            noneWeight = 400;
        }

        else {
            noneWeight = 1200;
        }

        uint256 total = 7000 + noneWeight;

        if (Random.randRange(rndCtx, 1, total) <= noneWeight) {
            return E_Female_Hat_Hair.None;
        }

        bytes memory packed = hex"";
        return E_Female_Hat_Hair(Random.selectRandomTrait(rndCtx, packed, 7000));
    }

    function selectFemaleHeadwear(RandomCtx memory rndCtx) external pure returns (E_Female_Headwear) {
        bytes memory packed = hex"";
        return E_Female_Headwear(Random.selectRandomTrait(rndCtx, packed, 9997));
    }

    function selectFemaleEyeWear(RandomCtx memory rndCtx) external pure returns (E_Female_Eye_Wear) {
        bytes memory packed = hex"";
        return E_Female_Eye_Wear(Random.selectRandomTrait(rndCtx, packed, 9682));
    }
}
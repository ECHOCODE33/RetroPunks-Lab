// SPDX-License-Identifier: MIT
pragma solidity ^0.8.30;

import { IMaleProbs } from './interfaces/IMaleProbs.sol';
import { E_Male_Skin, E_Male_Eyes, E_Male_Face, E_Male_Chain, E_Male_Earring, E_Male_Scarf, E_Male_Facial_Hair, E_Male_Mask, E_Male_Hair, E_Male_Hat_Hair, E_Male_Headwear, E_Male_Eye_Wear, E_Female_Skin, E_Female_Eyes, E_Female_Face, E_Female_Chain, E_Female_Earring, E_Female_Scarf, E_Female_Mask, E_Female_Hair, E_Female_Hat_Hair, E_Female_Headwear, E_Female_Eye_Wear, E_Mouth } from "./common/Enums.sol";
import { TraitsContext} from './common/Structs.sol';
import { RandomCtx, Random } from "./libraries/Random.sol";
import { TraitsLib } from "./libraries/TraitsLib.sol";

/**
 * @author ECHO
 */

contract MaleProbs is IMaleProbs {

    function selectMaleSkin(RandomCtx memory rndCtx) external pure returns (E_Male_Skin) {
        bytes memory packed = hex"0000002300AF000C002D0037000E02EE02EE02EE02EE02EE02EE02EE02EE02EE02EE02EE02EE000800160010003C0012001E000A0050012C0078";
        return E_Male_Skin(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleEyes(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Male_Eyes) {
        if (TraitsLib.maleIsGhost(ctx)) {
            // 50/50 chance for Ghost Eyes
            return Random.randBool(rndCtx, 5000) ? E_Male_Eyes.Ghost_Left : E_Male_Eyes.Ghost_Right;
        }

        bytes memory packed = hex"000000320064000B000B000B000B000B000B000B000B000B00FA101D00190019101D007D01F401F4004C";
        return E_Male_Eyes(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleFace(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Male_Face) {
        bytes memory packed;
        uint256 totalWeight;
        
        if (TraitsLib.maleIsHuman(ctx) || TraitsLib.maleIsZombie(ctx)) {
            // Human/Zombie weights
            packed = hex"249500120010004B00100023000A000A0008000C00140008000C00C8003C000C000A000E00320016";
            totalWeight = 9973;
        } 

        else if (TraitsLib.maleIsApe(ctx) || TraitsLib.maleIsYeti(ctx) || TraitsLib.maleIsZombieApe(ctx)) {
            // Ape weights (Only Mole, Bandage, War Paint, Bionics, Cybereyes)
            packed = hex"249500000000004B00100000000A000A0000000000000000000C00C80000000C0000000000320000";
            totalWeight = 9750;
        } 

        else if (TraitsLib.maleIsAlien(ctx) || TraitsLib.maleIsDemon(ctx) || TraitsLib.maleIsGhost(ctx) || TraitsLib.maleIsGlitch(ctx) || TraitsLib.maleIsGoblin(ctx) || TraitsLib.maleIsPumpkin(ctx) || TraitsLib.maleIsSkeleton(ctx) || TraitsLib.maleIsVampire(ctx)) {
            // All other weights (Only Mole, Tattoos, Bandage)
            packed = hex"249500120010004B00000023000000000000000C00140008000000C800000000000A000E00000016";
            totalWeight = 9795;
        }

        else {
            return E_Male_Face.None;
        }

        return E_Male_Face(Random.selectRandomTrait(rndCtx, packed, totalWeight));
    }

     function selectMaleChain(RandomCtx memory rndCtx) external pure returns (E_Male_Chain) {
        bytes memory packed = hex"232800FA0019004B008C015E000A00320064";
        return E_Male_Chain(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleEarring(RandomCtx memory rndCtx) external pure returns (E_Male_Earring) {
        bytes memory packed = hex"232800FA0019004B008C015E000A00320064";
        return E_Male_Earring(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleScarf(RandomCtx memory rndCtx) external pure returns (E_Male_Scarf) {
        bytes memory packed = hex"26AC0028001E001E";
        return E_Male_Scarf(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleFacialHair(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Male_Facial_Hair) {
        if (!TraitsLib.maleIsHuman(ctx) && !TraitsLib.maleIsZombie(ctx) && !TraitsLib.maleIsGhost(ctx)) {
            return E_Male_Facial_Hair.None;
        }

        bytes memory packed;
        uint256 totalWeight;

        if (TraitsLib.maleIsGhost(ctx)) {
            // Ghost Weights (Shadow traits only)
            packed = hex"";
            totalWeight = 5387;
        } 
        
        else {
            // Normal Weights (Human/Zombie)
            packed = hex"";
            totalWeight = 10000;
        }

        return E_Male_Facial_Hair(Random.selectRandomTrait(rndCtx, packed, totalWeight));
    }

    function selectMaleMask(RandomCtx memory rndCtx) external pure returns (E_Male_Mask) {
        bytes memory packed = hex"253A001E001E001E001E001E002800190019001900190019001900190019000F000F000A000A000A000A000A";
        return E_Male_Mask(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleHair(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Male_Hair) {
        uint256 noneWeight;

        if (TraitsLib.maleIsRobot(ctx) || TraitsLib.maleIsMummy(ctx) || TraitsLib.maleIsVampire(ctx)) {
            return E_Male_Hair.None;
        } 
        
        else if (TraitsLib.maleIsHuman(ctx) || TraitsLib.maleIsZombie(ctx)) {
            noneWeight = 400;
        } 
        
        else {
            noneWeight = 1200;
        }

        uint256 total = 9579 + noneWeight;
        
        if (Random.randRange(rndCtx, 1, total) <= noneWeight) {
            return E_Male_Hair.None;
        }

        bytes memory packed = hex"";
        return E_Male_Hair(Random.selectRandomTrait(rndCtx, packed, 9579));
    }

    function selectMaleHatHair(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Male_Hat_Hair) {
        uint256 noneWeight;
        
        if (TraitsLib.maleIsRobot(ctx) || TraitsLib.maleIsMummy(ctx) || TraitsLib.maleIsVampire(ctx)) {
            return E_Male_Hat_Hair.None;
        }

        else if (TraitsLib.maleIsHuman(ctx) || TraitsLib.maleIsZombie(ctx)) {
            noneWeight = 400;
        }

        else {
            noneWeight = 800;
        }
        
        uint256 total = 7580 + noneWeight;
        
        if (Random.randRange(rndCtx, 1, total) <= noneWeight) {
            return E_Male_Hat_Hair.None;
        }

        bytes memory packed = hex"";
        return E_Male_Hat_Hair(Random.selectRandomTrait(rndCtx, packed, 7580));
    }

    function selectMaleHeadwear(RandomCtx memory rndCtx) external pure returns (E_Male_Headwear) {
        bytes memory packed = hex"";
        return E_Male_Headwear(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMaleEyeWear(RandomCtx memory rndCtx) external pure returns (E_Male_Eye_Wear) {
        bytes memory packed = hex"";
        return E_Male_Eye_Wear(Random.selectRandomTrait(rndCtx, packed, 10000));
    }

    function selectMouth(TraitsContext calldata ctx, RandomCtx memory rndCtx) external pure returns (E_Mouth) {   
        if (TraitsLib.isFemale(ctx)) {
            bytes memory packed = hex"1FDB007D001E00C800320096009600FA0032000F0014000F001E0014002300320064000A001900640064012C0014";
            return E_Mouth(Random.selectRandomTrait(rndCtx, packed, 10000));
        }

        else if (TraitsLib.maleHasBlackFacialHair(ctx)) {
            bytes memory packed = hex"1FDB007D001E00C800320096009600FA0032000F0014000000000000000000320064000A001900640064012C0014";
            return E_Mouth(Random.selectRandomTrait(rndCtx, packed, 9900));
        }

        else {
            bytes memory packed = hex"1FDB007D001E00C800320096009600FA0032000F0014000000000000000000320064000A001900000000012C0014";
            return E_Mouth(Random.selectRandomTrait(rndCtx, packed, 9700));
        }
    }
}
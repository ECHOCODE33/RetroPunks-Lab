...
library Division {
    ...
    function divisionStr(uint8 decimalPlaces, int256 numerator, int256 denominator) internal pure returns (string memory) {
    ...
    function _division(uint8 decimalPlaces, int256 numerator, int256 denominator)
        internal
        pure
        returns (int256 quotient, int256 remainder, string memory result)
    {
        unchecked {
            ...
            result = string(abi.encodePacked(quotient.toString(), ".", _numToFixedLengthStr(decimalPlaces, remainder)));
        }
    }

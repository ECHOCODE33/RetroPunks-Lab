...
import { E_Female_Chain, E_Female_Earring, E_Female_Eye_Wear, E_Female_Eyes, E_Female_Face, E_Female_Hair, E_Female_Hat_Hair, E_Female_Headwear, E_Female_Mask, E_Female_Scarf, E_Female_Skin, E_Male_Chain, E_Male_Earring, E_Male_Eye_Wear, E_Male_Eyes, E_Male_Face, E_Male_Facial_Hair, E_Male_Hair, E_Male_Hat_Hair, E_Male_Headwear, E_Male_Mask, E_Male_Scarf, E_Male_Skin, E_Mouth } from "./global/Enums.sol";
...
contract Rarities {
    ...
    function selectMaleFace(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Male_Face) {
        ...
        if (TraitLogic.maleIsHuman(ctx) || TraitLogic.maleIsZombie(ctx)) {
            // Human/Zombie weights
            packed = M_FACE_A;
            totalWeight = 9973;
        } else if (TraitLogic.maleIsApe(ctx) || TraitLogic.maleIsYeti(ctx) || TraitLogic.maleIsZombieApe(ctx)) {
            // Ape weights (Only Mole, Bandage, War Paint, Bionics, Cybereyes)
            packed = M_FACE_B;
            totalWeight = 9750;
        } else if (TraitLogic.maleIsAlien(ctx) || TraitLogic.maleIsDemon(ctx) || TraitLogic.maleIsGhost(ctx) || TraitLogic.maleIsGlitch(ctx) || TraitLogic.maleIsGoblin(ctx) || TraitLogic.maleIsPumpkin(ctx) || TraitLogic.maleIsSkeleton(ctx) || TraitLogic.maleIsVampire(ctx)) {
    ...
    function selectMaleFacialHair(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Male_Facial_Hair) {
        if (!TraitLogic.maleIsHuman(ctx) && !TraitLogic.maleIsZombie(ctx) && !TraitLogic.maleIsGhost(ctx)) {
    ...
    function selectMaleHair(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Male_Hair) {
        ...
        if (TraitLogic.maleIsRobot(ctx) || TraitLogic.maleIsMummy(ctx) || TraitLogic.maleIsVampire(ctx)) {
            return E_Male_Hair.None;
        } else if (TraitLogic.maleIsHuman(ctx) || TraitLogic.maleIsZombie(ctx)) {
            noneWeight = 400;
        } else {
            noneWeight = 1200;
        }
    ...
    function selectFemaleEyes(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Female_Eyes) {
        if (TraitLogic.femaleIsGhost(ctx)) {
    ...
    function selectFemaleFace(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Female_Face) {
        ...
        if (TraitLogic.femaleIsHuman(ctx) || TraitLogic.femaleIsZombie(ctx)) {
            // Human/Zombie weights
            packed = F_FACE_A;
            totalWeight = 10000;
        } else if (TraitLogic.femaleIsApe(ctx) || TraitLogic.femaleIsZombieApe(ctx)) {
            // Ape weights (Only Mole, Bandage, War Paint, Bionics, Cybereyes)
            packed = F_FACE_B;
            totalWeight = 10000;
        } else if (TraitLogic.femaleIsAlien(ctx) || TraitLogic.femaleIsDemon(ctx) || TraitLogic.femaleIsGhost(ctx) || TraitLogic.femaleIsGlitch(ctx) || TraitLogic.femaleIsGoblin(ctx) || TraitLogic.femaleIsSkeleton(ctx) || TraitLogic.femaleIsVampire(ctx)) {
            // All other weights (Only Mole, Tattoos, Bandage)
            packed = F_FACE_C;
            totalWeight = 10000;
        } else {
    ...
    function selectFemaleHair(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Female_Hair) {
        ...
        if (TraitLogic.femaleIsRobot(ctx) || TraitLogic.femaleIsMummy(ctx) || TraitLogic.femaleIsVampire(ctx)) {
            return E_Female_Hair.None;
        } else if (TraitLogic.femaleIsHuman(ctx) || TraitLogic.femaleIsZombie(ctx)) {
            noneWeight = 400;
        } else {
            noneWeight = 1200;
        }
    ...
    function selectMouth(TraitsContext memory ctx, LibPRNG.PRNG memory prng) internal pure returns (E_Mouth) {
        if (TraitLogic.isFemale(ctx)) {
            return E_Mouth(selectRandomTrait(prng, MOUTH_A, 10000));
        } else if (TraitLogic.maleHasBlackFacialHair(ctx)) {

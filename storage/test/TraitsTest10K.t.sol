...
contract TraitsSimulationTest is Test {
    ...
    function setUp() public {
        traitsContract = new Traits(new MaleProbs(), new FemaleProbs());
        stats = new CumulativeStats();
    }

    function testFuzz_Simulate(uint256 seed) public {
        // We use a specific range for the simulation
        uint16 tokenIdSeed = uint16(bound(seed, 1, 10000));
        uint256 globalSeed = 987654321;

        try traitsContract.generateAllTraits(tokenIdSeed, 0, globalSeed) returns (TraitsContext memory ctx) {
        ...
        // Foundry runs fuzz tests in parallel/random order, so we check if we've hit our target
        if (stats.successfulRuns() == 10000) {
            _printReport();
        }
    }
    ...
    function _logGroup(string memory key, string memory label, uint256 divisor) internal view {
        ...
        for (uint i = 0; i <= max; i++) {
            ...
            uint256 p = (count * 10000) / divisor;
            string memory dStr = (p % 100) < 10 ? string.concat("0", vm.toString(p % 100)) : vm.toString(p % 100);
            console.log(string.concat("  Idx [", vm.toString(i), "]: ", vm.toString(count), " (", vm.toString(p/100), ".", dStr, "%)"));
        }
    }
}
...
contract RetroPunks is ERC721SeaDropPausable {
    ...
    constructor(ISVGRenderer _rendererParam, bytes32 _committedGlobalSeedHashParam, bytes32 _committedShufflerSeedHashParam, uint _maxSupplyParam, address[] memory allowedSeaDropParam) ERC721SeaDropPausable("RetroPunks", "RPNKS", allowedSeaDropParam) {
    ...
    function resetTokenName(uint256 tokenId) external onlyTokenOwner(tokenId) notSpecial(tokenId) {
        string memory defaultName = string.concat('#', Utils.toString(tokenId));

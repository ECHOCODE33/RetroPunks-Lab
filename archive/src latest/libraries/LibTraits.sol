...
library LibTraits {
    ...
    function maleIsHuman(
        TraitsContext memory traits
    ) internal pure returns (bool) {
        return
            uint8(traits.maleSkin) >= uint8(E_Male_Skin.Human_1) &&
            uint8(traits.maleSkin) <= uint8(E_Male_Skin.Human_12);
    }

    function maleIsAlien(
        TraitsContext memory traits
    ) internal pure returns (bool) {

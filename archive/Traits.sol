...
contract Traits is ITraits {
    ...
    function generateTraitsContext(uint16 _tokenIdSeed, uint8 _backgroundIndex, uint256 _globalSeed) external view returns (TraitsContext memory) {
        ...
        if (_tokenIdSeed < NUM_SPECIAL_1S) {
            ...
            // Early Return for "Super Specials" (The first 7)
            if (_tokenIdSeed < 7) {
                // These don't even need an array allocated if they are pre-rendered
                return traits; 
            }

            // Specials 8 and above need their traits rendered
            traits.traitsToRender = new TraitToRender[](2); // Small allocation saves gas
            traits.background = E_Background(_backgroundIndex);
            _addBackground(traits);
            _addSpecial(traits, _tokenIdSeed); 

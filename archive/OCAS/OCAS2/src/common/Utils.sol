...
library Utils {
    ...
    */
    ...
    function allocateBuffer(uint size) internal pure returns (bytes memory buffer) {
    ...
    function concat(bytes memory buffer, bytes memory c1, bytes memory c2, bytes memory c3, bytes memory c4, bytes memory c5, bytes memory c6, bytes memory c7, bytes memory c8, bytes memory c9, bytes memory c10) internal pure {
        concat(buffer, c1, c2, c3, c4, c5, c6, c7, c8, c9);
        concat(buffer, c10);
    }

    function concat(bytes memory buffer, bytes memory c1, bytes memory c2, bytes memory c3, bytes memory c4, bytes memory c5, bytes memory c6, bytes memory c7, bytes memory c8, bytes memory c9, bytes memory c10, bytes memory c11) internal pure {
        concat(buffer, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10);
        concat(buffer, c11);
    }

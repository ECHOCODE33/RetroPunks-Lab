...
contract Traits is ITraits {
    ...
    function addTraitToRender(TraitsContext memory traitsContext, E_TraitsGroup traitGroup, uint8 traitIndex) internal pure {
        int8 traitId = -1;
        
        if (traitGroup == E_TraitsGroup.E_2_Tattoos_Group) {
            traitId = int8(int(++traitsContext.tattooId));
        } else if (traitGroup == E_TraitsGroup.E_4_Jewellery_Group) {
            traitId = int8(int(++traitsContext.jewelryId));
        } else if (traitGroup == E_TraitsGroup.E_3_Clothes_Group || traitGroup == E_TraitsGroup.E_3b_Clothes_Group) {
            traitId = int8(int(++traitsContext.clothesId));
        }

        TraitToRender memory traitToRender;
        traitToRender.traitGroup = traitGroup;
        traitToRender.traitIndex = traitIndex;
        traitToRender.traitId = traitId;
        traitToRender.hasFiller = false;
        traitToRender.fillerRenderedAtTheEnd = false;
        traitsContext.traitsToRender[traitsContext.traitsToRenderLength] = traitToRender;
        traitsContext.traitsToRenderLength++;
    }
    ...
    function addFillerTrait(TraitsContext memory traitsContext, E_TraitsGroup traitGroup, uint8 traitIndex, bool fillerRenderedAtTheEnd) internal pure {
        FillerTrait memory filler;
        filler.traitGroup = traitGroup;
        filler.traitIndex = traitIndex; 
        
        traitsContext.traitsToRender[traitsContext.traitsToRenderLength - 1].hasFiller = true;
        traitsContext.traitsToRender[traitsContext.traitsToRenderLength - 1].filler = filler;
        traitsContext.traitsToRender[traitsContext.traitsToRenderLength - 1].fillerRenderedAtTheEnd = fillerRenderedAtTheEnd;
    }
}
...
contract Traits is ITraits {
    ...
    function generateTraitsContext(uint16 _tokenIdSeed, uint8 _backgroundIndex, uint256 _globalSeed) external view returns (TraitsContext memory) {
        ...
        if (traits.sex == E_Sex.Male) {
            ...
            MaleTraits memory m = MALE_PROBS_CONTRACT.selectAllMaleTraits(traits, prng);
            ...
            FemaleTraits memory f = FEMALE_PROBS_CONTRACT.selectAllFemaleTraits(traits, prng);
        ...
        if (!LibTraits.femaleHasMask(traits) && !LibTraits.maleHasMask(traits)) {
            _addMouth(traits);
        }
